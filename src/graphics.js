...
export default {
	...
	// 	let self = this;
	// 	let axesHelper = new THREE.AxesHelper(this.appSettings.axesHelper.axisLength);
	...
	rotateGeometryAboutLine: function(geometry, axisPt1, axisPt2, angle) {
		...
		for (let i = 0; i < geometry.vertices.length; i++) {
			geometry.vertices[i].set(this.rotatePointAboutLine(geometry.vertices[i], axisPt1, axisPt2, angle).x, this.rotatePointAboutLine(geometry.vertices[i], axisPt1, axisPt2, angle).y, this.rotatePointAboutLine(geometry.vertices[i], axisPt1, axisPt2, angle).z );
		}
	...
	showPoints: function(geometry, scene, color, opacity) {
		...
		for (let i = 0; i < geometry.vertices.length; i++) {
			this.showPoint(geometry.vertices[i], scene, color, opacity);
		}
	},
	...
	labelPoint: function(pt, label, scene, color) {
		...
		let self = this;
		if (this.appSettings.font.enable) {
			color = color || 0xff0000;
	...
	labelAxes: function(scene) {
		...
		let self = this;
		if (this.appSettings.font.enable) {
			let textGeometry = new THREE.TextGeometry('Y', this.appSettings.font.fontStyle);
			let textMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00 });
			let mesh = new THREE.Mesh(textGeometry, textMaterial);
			textGeometry.translate(0, this.appSettings.axesHelper.axisLength, 0);
			scene.add(mesh);
			
			textGeometry = new THREE.TextGeometry('X', this.appSettings.font.fontStyle);
			textMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000 });
			mesh = new THREE.Mesh(textGeometry, textMaterial);
			textGeometry.translate(this.appSettings.axesHelper.axisLength, 0, 0);
			scene.add(mesh);
			
			textGeometry = new THREE.TextGeometry('Z', this.appSettings.font.fontStyle);
			textMaterial = new THREE.MeshBasicMaterial({ color: 0x0000ff });
			mesh = new THREE.Mesh(textGeometry, textMaterial);
			textGeometry.translate(0, 0, this.appSettings.axesHelper.axisLength);
			scene.add(mesh);
		}
	},
	...
	setCameraDirection: function(camera, pt) {
		camera.lookAt(pt);
	},
